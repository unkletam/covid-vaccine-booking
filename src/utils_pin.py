...
import tabulate, copy, time, datetime, requests, sys, os, json

BOOKING_URL = "https://cdn-api.co-vin.in/api/v2/appointment/schedule"
BENEFICIARIES_URL = "https://cdn-api.co-vin.in/api/v2/appointment/beneficiaries"
CALENDAR_URL = "https://cdn-api.co-vin.in/api/v2/appointment/sessions/public/calendarByPin?pincode={0}&date={1}"
...
def check_calendar(request_header, vaccine_type, pincode, minimum_slots, min_age_booking):
    """
    This function
    ...
    try:
        ...
        resp = requests.get(base_url.format(pincode, tomorrow), headers=request_header)
        ...
        if resp.status_code == 401:
            ...
            resp = resp.json()
            print(
                f"Centers available in area with pincode - {pincode} from {tomorrow} as of {today.strftime('%Y-%m-%d %H:%M:%S')}: {len(resp['centers'])}")
            print("test5")
            ...
            '''for i in set([item['district_name'] for item in options]):
                beep_pin(150)'''
...
def check_and_book_pin(request_header, beneficiary_dtls, pincode, minimum_slots):
    """
    This function
    ...
    """
    try:
        min_age_booking = get_min_age_pin(beneficiary_dtls)
        vaccine_type = [beneficiary['vaccine'] for beneficiary in beneficiary_dtls][0]
        options = check_calendar(request_header, vaccine_type, pincode, minimum_slots, min_age_booking)
        print("test0")
...
def get_beneficiaries_pin(request_header):
    """
    This function
...
def get_min_age_pin(beneficiary_dtls):
    """
...
def generate_token_OTP_pin(mobile):
    """
